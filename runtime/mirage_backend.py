...
from runtime.bwrap import BwrapBackend
...
class MirageSandboxBackend(MirageBackend, SandboxBackendProtocol):
    """`MirageBackend` + auto-syncing bwrap sandbox for `execute()`.

    Every `execute` mirrors mirage → sandbox, runs in bwrap, mirrors back
    (full mirror; adds, updates, deletes). Sync is resource-agnostic — works
    over `DiskResource`, `S3Resource`, etc.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._bwrap = BwrapBackend(root_dir=str(self._sandbox), image=bwrap_image, timeout=timeout)
    ...
    def execute(self, command: str, *, timeout: int | None = None) -> ExecuteResponse:
        pull(self._io, self._loop, self._sandbox)
        response = self._bwrap.execute(command, timeout=timeout)
        push(self._io, self._loop, self._sandbox)
        return response
