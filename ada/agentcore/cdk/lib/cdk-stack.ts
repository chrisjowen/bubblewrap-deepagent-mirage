...
export class AgentCoreStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    // Grant every runtime read/write access to the mirage S3 bucket. Bucket + prefix
    // are configured on the runtime via envVars (S3_BUCKET, S3_KEY_PREFIX). Keep the
    // bucket name here in sync with agentcore.json envVars.
    const MIRAGE_BUCKET = 'mirage-test-chris';
    for (const env of this.application.environments.values()) {
      env.runtime.role.addToPrincipalPolicy(
        new iam.PolicyStatement({
          actions: ['s3:ListBucket', 's3:GetBucketLocation'],
          resources: [`arn:aws:s3:::${MIRAGE_BUCKET}`],
        })
      );
      env.runtime.role.addToPrincipalPolicy(
        new iam.PolicyStatement({
